...
exports.getTokenMarketData = async (tokenAddress) => {
...
exports.generateRandomChange = () => {
  return (Math.random() * 40 - 20).toFixed(2);
};

// Get token price history formatted for charts
exports.getTokenPriceHistoryForCharts = async (tokenId) => {
  const intervals = {
...
exports.generateMockPriceHistory = (limit, timeframe) => {
  const result = [];
  const now = new Date();
...
exports.updateTokenMarketData = async () => {
  try {
    // Get all tokens
