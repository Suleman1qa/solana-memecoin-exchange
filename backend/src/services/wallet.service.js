...
const walletService = {
  // Create a new wallet
  async createWallet(userId, type, label) {
    // Generate keypair
  ...
  async addTokenToWallet(walletId, tokenId) {
  ...
  async getWalletWithTokenDetails(wallet) {
    // Fetch token details
  ...
  async transferBetweenWallets(userId, sourceWalletId, destWalletId, tokenId, amount) {
    // Validate amount
  ...
  // Deposit funds
  async depositFunds(userId, walletId, tokenId, amount, txHash) {
    // Validate amount
    const amountBN = new BigNumber(amount);
    if (amountBN.isNaN() || amountBN.isLessThanOrEqualTo(0)) {
      throw new AppError('Invalid amount', 400);
  ...
  // Withdraw funds
  async withdrawFunds(userId, walletId, tokenId, amount, destinationAddress) {
    // Validate amount
    const amountBN = new BigNumber(amount);
    if (amountBN.isNaN() || amountBN.isLessThanOrEqualTo(0)) {
      throw new AppError('Invalid amount', 400);
  ...
  // Swap tokens
  async swapTokens(userId, walletId, fromTokenId, toTokenId, amount, slippageTolerance) {
    // Validate amount
    const amountBN = new BigNumber(amount);
    if (amountBN.isNaN() || amountBN.isLessThanOrEqualTo(0)) {
      throw new AppError('Invalid amount', 400);
