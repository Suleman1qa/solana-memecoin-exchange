...
const marketService = {
  ...
  getOrderBook: async (pairId, depth = 50) => {
    ...
    const aggregatedBids = marketService.aggregateOrdersByPrice(bids);
    const aggregatedAsks = marketService.aggregateOrdersByPrice(asks);
  ...
  // Place order
  placeOrder: async (userId, pairId, type, side, amount, price, stopPrice) => {
    // Get trading pair
    const pair = await TradingPair.findById(pairId)
      .populate('baseToken')
      .populate('quoteToken');
    ...
    if (type === 'MARKET') {
      try {
        await marketService.executeMarketOrder(order, pair, wallet);
      } catch (error) {
        // If market order execution fails, unlock funds and mark order as rejected
        await marketService.cancelOrder(order._id);
        throw error;
      }
    } else {
      // For limit orders, try to match with existing orders
      await marketService.matchLimitOrder(order, pair, wallet);
    }
  ...
  executeMarketOrder: async (order, pair, wallet) => {
    ...
    await marketService.updateBalancesAfterTrade(order, pair, wallet);
    
    return order;
  },

  // Match limit order
  matchLimitOrder: async (order, pair, wallet) => {
  ...
  updateBalancesAfterTrade: async (order, pair, wallet) => {
    if (order.side === 'BUY') {
      // Add base token
      let baseTokenBalance = wallet.balances.find(b => 
        b.token.toString() === pair.baseToken._id.toString()
      );
