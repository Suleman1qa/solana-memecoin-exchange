...
class TokenListener {
  ...
  async setupTokenProgramSubscription() {
    try {
      ...
      this.lastSignature =
        await this.tokenProcessor.getLastProcessedSignature();
  ...
  addToProcessingQueue(tokenData) {
    ...
    logger.debug(
      `Added token ${tokenData.address} to processing queue. Queue size: ${this.processingQueue.length}`
    );
  }
  ...
  async processQueue() {
    ...
    try {
      ...
      logger.debug(
        `Processing token ${tokenData.address} from queue. Remaining: ${this.processingQueue.length}`
      );

      await this.processTokenAccount(
        tokenData.address,
  ...
  async processTokenAccount(address, accountInfo, slot) {
    try {
      ...
      const isBeingProcessed =
        await this.tokenProcessor.isTokenBeingProcessed(address);
      if (isBeingProcessed) {
      ...
      if (mintAuthorityOption !== 1) {
        logger.debug(
          `Token ${address} has no mint authority. Likely not a memecoin. Skipping.`
        );
        return;
      }
      ...
      if (decimals > config.tokenFilters.maxDecimals) {
        logger.debug(
          `Token ${address} has too many decimals (${decimals}). Skipping.`
        );
        return;
      }

      // Get additional token information
      const tokenInfo = await this.tokenProcessor.getTokenInfo(
        address,
        decimals
      );
      ...
      const isMemecoin = await this.tokenProcessor.checkIfMemecoin(
        address,
        tokenInfo
      );
      if (!isMemecoin) {
        logger.debug(
          `Token ${address} doesn't meet memecoin criteria. Skipping.`
        );
        return;
      }
  ...
  async pollHistoricalTokens() {
    try {
      ...
      logger.info(
        `Found ${recentTokens.length} recent tokens to check for related tokens`
      );
      ...
      for (const token of recentTokens) {
        try {
          ...
          // Use rate limiter for getting signatures
          const signatures = await rateLimiter.enqueue(() =>
          ...
          // Process each signature with rate limiting
          for (const sig of signatures) {
            try {
              // Use rate limiter for getting transaction
              ...
              if (tx && tx.meta && tx.meta.postTokenBalances) {
                for (const balance of tx.meta.postTokenBalances) {
                  if (
                    balance.mint &&
                    !this._processedTokens.has(balance.mint)
                  ) {
                    this._processedTokens.add(balance.mint);
                    await this.processTokenAccount(balance.mint, null, tx.slot);
                  }
                }
              }
            } catch (error) {
              if (error.message.includes("429")) {
                logger.warn(
                  "Rate limited while processing transaction, backing off..."
                );
          ...
          if (error.message.includes("429")) {
            logger.warn(
              "Rate limited while getting signatures, backing off..."
            );
            await new Promise((resolve) => setTimeout(resolve, 5000));
          } else {
      ...
      logger.error("Error polling historical tokens:", error);
    }
  }
