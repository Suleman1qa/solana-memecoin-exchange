...
const initSolanaConnection = async (retries = 5) => {
  try {
    ...
    if (retries > 0) {
      logger.warn(
        `Solana connection failed, retrying (${retries}): ${error.message}`
      );
      await new Promise((res) => setTimeout(res, 2000));
      return initSolanaConnection(retries - 1);
    } else {
      logger.error(
        "Failed to connect to Solana after multiple attempts:",
        error
      );
      throw error;
    }
...
const startBlockchainListener = async () => {
  ...
  const metadataListener = new MetadataListener(
    solanaConnection,
    tokenProcessor
  );
  const liquidityListener = new LiquidityListener(
    solanaConnection,
    tokenProcessor
  );
